...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length) return message.reply("Please specify a command to enable!")
    ...
    if (!disabled.includes(command.name)) return message.reply("That command is already enabled!")
    disabled = disabled.filter(c => c !== command.name)
    await disabledCommands.set(message.guild.id, disabled)
    return message.channel.send(`Successfully enabled command \`${command.name}\`.`)
  }
}
