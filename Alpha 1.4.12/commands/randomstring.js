...
execute(message, bot, args, command, prefix, sendEmbed, sendErrorEmbed, sendUnsuccessfulAction, cooldownrandomstring) {
	if (cooldownrandomstring.has(message.author.id)) return message.channel.send(`${message.author}, you have already used this command in the past 5 seconds.`)
	cooldownrandomstring.add(message.author.id);
	setTimeout(() => {
		cooldownrandomstring.delete(message.author.id);
	}, 5000);
  args = args.slice(1);
